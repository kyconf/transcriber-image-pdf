...
const RANGE_NAME = 'Sheet19!A1'; // Replace with your range
...

{
  ...
  "correct_answer": "[Letter]"
    ...
    await appendToSheet(valuesToAppend); // Call the append function
...
async function appendToSheet(values) {
  try {
    const sheet = await sheets.spreadsheets.values.get({
      ...
      range: RANGE_NAME, // Adjust based on your target sheet
    });
    ...
    const targetRange = `Sheet19!A${targetRow}`; // Define the cell to start inserting
...
app.post('/transcribe', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,  
      message: 'Processed all images in the folder.',
      data: responses,
    });
  } catch (error) {
    console.error('Error processing images from folder:', error.message);
    return res.status(500).json({
      success: false,
      message: 'Failed to process images from folder.',
      error: error.message,
    });
  } 
});
...
async function processPdfAndUpload() {
  try {
    createDefaultSheet(SPREADSHEET_ID);
    await clearProcessingQueue(); // 🚀 Ensure everything is clean before processing
    const images = await convertPdfToImages(PDF_FILE_PATH, OUTPUT_DIR);
    
    for (const image of images) {
      await uploadFileToDrive(image);
    }

    console.log("✅ All images uploaded to Google Drive.");
...
try {
  const files = await listFilesInFolder(FOLDER_PDF);
